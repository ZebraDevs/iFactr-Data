    ...
    public partial class ZipEntry
    {
        ...
        private void InternalExtract(string baseDir, Stream outstream, string password)
        {
            ...
            try
            {
                ...
                {
                    output.Position = 0;
                    Device.File.Save(targetFileName, output, EncryptionMode.NoEncryption);
                    ...
                    if (checkLaterForResetDirTimes)
                    {
                        ...
                        if (this.FileName.IndexOf('/') != -1)
                        {
                            ...
                            {
                                _SetTimes(System.IO.Path.GetDirectoryName(targetFileName), false);
                            }
                        }
                    }
                }
        ...
        private bool ValidateOutput(string basedir, Stream outstream, out string outFileName)
        {
            if (basedir != null)
            {
                ...
                if (_container.ZipFile.FlattenFoldersOnExtract)
                    outFileName = MonoCross.Utilities.Compatibility.Path.Combine(basedir,
                                              (f.IndexOf('/') != -1) ? System.IO.Path.GetFileName(f) : f);
                else
                    outFileName = MonoCross.Utilities.Compatibility.Path.Combine(basedir, f);
