...
namespace Ionic.Zip
{
    public partial class ZipFile
    {
            ...
            if (preserveDirHierarchy)
            {
                foreach (var f in fileNames)
                {
                    ...
                    if (directoryPathInArchive != null)
                    {
                        ...
                        string s = MonoCross.Utilities.Compatibility.Path.GetFullPath(MonoCross.Utilities.Compatibility.Path.Combine(directoryPathInArchive, MonoCross.Utilities.Compatibility.Path.GetDirectoryName(f)));
                        this.AddFile(f, s);
                    }
                    else
                        this.AddFile(f, null);
        ...
        private void RemoveEntryForUpdate(string entryName)
        {
            ...
            {
                directoryPathInArchive = System.IO.Path.GetDirectoryName(entryName);
                entryName = System.IO.Path.GetFileName(entryName);
            }
                                                  ...
                                                  int level)
        {
            ...
            if (level > 0)
            {
                ...
                dirForEntries = MonoCross.Utilities.Compatibility.Path.Combine(rootDirectoryPathInArchive, dirForEntries);
            }
