...
namespace Ionic.Zlib
{
    ...
    public class ParallelDeflateOutputStream : System.IO.Stream, IDisposable
    {
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                ...
                if (workitem.inputBytesAvailable == workitem.buffer.Length)
                {
                    ...
                    Device.Thread.QueueWorker(_DeflateOne, workitem);
        ...
        private void TraceOutput(TraceBits bits, string format, params object[] varParams)
        {
            if ((bits & _DesiredTrace) != 0)
            {
                lock (_outputLock)
                {
                    Debug.WriteLine(string.Format("{0:000} PDOS ", Environment.CurrentManagedThreadId) + format, varParams);
                }
            }
        }
